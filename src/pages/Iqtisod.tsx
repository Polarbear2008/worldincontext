...
const Iqtisod = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        setLoading(true);
        const data = await articlesService.getArticles({ category: 'Iqtisod', status: 'published' });
        setArticles(data);
      } catch (error) {
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto text-center">
              <div className="inline-flex items-center gap-2 bg-green-100 text-green-800 px-4 py-2 rounded-full text-sm font-medium mb-6">
                ...
                Iqtisod kategoriyasi
              </div>
              <h1 className="text-4xl md:text-6xl font-bold text-foreground mb-6">
                Iqtisod <span className="text-gradient">Maqolalari</span>
              </h1>
              <p className="text-xl text-muted-foreground mb-8 max-w-2xl mx-auto">
                Iqtisodiy nazariyalar, bozor tendentsiyalari va soddalashtirilgan moliyaviy tushunchalar
              </p>
            </div>
          </div>
        </section>
              ...
              {articles.length === 0 ? (
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {articles.map((article, index) => (
                    <Card
                      key={article.id}
                      className="shadow-elegant hover:shadow-glow hover-lift group relative overflow-hidden border-0 bg-gradient-to-br from-background to-green-50 animate-fade-in"
                      ...
                      <CardContent className="space-y-4">
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-border/30">
                          ...
                          <Button variant="ghost" size="sm" className="hover-scale group" asChild>
                            <a href={`/article/iqtisod/${article.id}`} className="story-link">
                              O'qish
